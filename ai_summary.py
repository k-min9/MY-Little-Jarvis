'''
장기메모리용 정보 압축
대화내용을 입력 받아서 유저와 캐릭터의 정보를 입력 받음
'''
import json
import re
...
def get_LLAMA3_prompt():
    ...
    messages = list() 
    content = """You're an expert assistant at identifying character traits from dialog. given dialog is a conversation between two people, {char} and {user}.

please respond with a JSON consisting of the formats {"type": char or user, "trait": Trait you've identified, "reason": optional, reason or sentence that make you to think that.}.

Example:
[
    {
        "type": "char",
        "trait" : "char loves to play music",
        "reason" : "char sometimes play music while working"
    },
    {
        "type": "char",
        "trait" : "char likes to eat cookie",
        "reason" : "I prefer cookie"
    },
    {
        "type" : "user",
        "trait" : "user sometimes wants like to be called sensei",
    }
]
...
"""
    # dialog = dialog + "Tell me all the traits of the {char} as best you can.\n"
    # dialog = dialog + "Tell me all the traits of the {user} as best you can.\n"
    dialog = dialog + "Tell me all the traits as best you can.\n"
...
def get_punctuation_sentences(texts):
    ...
    # print('punc_sen', sentences)
    return sentences

def get_trait_infos(text):
    ...
    # 3단계: 리스트를 반복하며 'type'과 'trait'이 포함된 딕셔너리만 필터링
    result = []
    for item in data_list:
        if "type" in item and "trait" in item:
            result.append(item)
...
if __name__ == "__main__":
    ...
    for j, reply in enumerate(process_stream(None, 'm9dev', 'arona', True, False)):
        pass
    trait_infos = get_trait_infos(reply)
    print(trait_infos)
    for trait_info in trait_infos:
        trait_type = trait_info['type']
        trait = trait_info['trait']
        trait_reason = ''
        if 'reason' in trait_info:
            trait_reason = trait_info['reason']
        
        print(trait_type + ":" + trait + "\nreason : " + trait_reason)
        # Todo Reason에 대한 번역기능 추가
